...
func (h *Handler) CreateGNB(w http.ResponseWriter, r *http.Request) {
	...
	gnb := h.Store.CreateGNB(req.MCC, req.MNC, req.TAC, req.GNBID, req.GNBIDBitLength, req.Name, req.SST, req.SD, slices)
	...
	if req.RawNGAPPDU != nil {
		...
		if len(req.NGSetupIEs) > 0 {
			...
			var berr error
			msg, berr = ngap.BuildNGSetupRequest(ngap.NGSetupRequestParams{
				MCC:              req.MCC,
				MNC:              req.MNC,
				TAC:              req.TAC,
				GNBID:            req.GNBID,
				GNBIDBitLength:   req.GNBIDBitLength,
				Name:             req.Name,
				SST:              req.SST,
				SD:               req.SD,
				Slices:           sliceArgs,
...
func (h *Handler) GetGNB(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, GNBStateResponse{
		...
		TAC:            gnb.TAC,
		GNBID:          gnb.GNBID,
		GNBIDBitLength: gnb.GNBIDBitLength,
		Name:           gnb.Name,
		SST:            gnb.SST,
		SD:             gnb.SD,
	})
